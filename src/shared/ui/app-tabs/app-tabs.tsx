...
type TAppTabsContextValue = {
  activeTabId: Optional<TAppTab['tabId']>
}

const AppTabsContext = createContext<Nullable<TAppTabsContextValue>>(null)

const AppTabs = ({ defaultKey, tabs, children, className, onChange }: AppTabsProps) => {
  const [activeTabId, setActiveTabId] = useState<Optional<TAppTab['tabId']>>(() => defaultKey || tabs?.at(0)?.tabId)
...
type AppTabContentProps = { tabId: TAppTab['tabId'] } & ChildrenProps

function AppTabContent({ tabId, children }: AppTabContentProps) {
